...
$should_load_data = !empty($kelas_id) || !empty($search) || !empty($mapel_id) || !empty($tanggal_awal);
...
if (!empty($_GET['kelas_id'])) {
    ...
    // Contoh query (pakai query kamu sendiri ya)
    $stmt = $pdo->prepare("SELECT * FROM siswa WHERE kelas_id = ?");
...
if ($tab === 'nilai') {
    ...
    // Grouping dan pengurutan harus paling akhir
    $query .= " GROUP BY n.id_nilai ORDER BY k.nama_kelas, s.nama";
...
// Jika search diisi, abaikan filter dropdown
...
<div>
    ...
    <select name="mapel_id"
        ...
        <!-- Tanggal (jika absensi) -->
        <?php if ($tab === 'absensi'): ?>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Pilih Tanggal</label>
                <input type="date" name="tanggal" value="<?= htmlspecialchars($tanggal) ?>" 
                    class="w-full rounded-lg border-gray-300 focus:ring-[#191970] focus:border-[#191970]" onchange="this.form.submit()">
            </div>
        <?php endif; ?>
...
<div class="mb-4 flex justify-between items-center">
    <!-- Navigasi Tab -->
    <div class="inline-flex rounded-lg shadow-sm overflow-hidden border border-[#4169E1] bg-white">
        <a href="?tab=absensi"
           class="px-4 py-2 text-sm font-medium <?= $tab === 'absensi' ? 'bg-[#191970] text-white' : 'text-[#191970] hover:bg-[#4169E1]' ?>">
            Absensi
        </a>
        <a href="?tab=nilai"
           class="px-4 py-2 text-sm font-medium <?= $tab === 'nilai' ? 'bg-[#191970] text-white' : 'text-[#191970] hover:bg-[#4169E1]' ?>">
            Nilai
        </a>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Reset dropdown kalau search aktif
    if (searchInput) {
    ...
    // Saat KELAS diganti → reset search & siswa, lalu submit
    if (kelasSelect) {
    ...
    // Saat MAPEL atau SISWA diganti → reset search & submit
    [mapelSelect, siswaSelect].forEach(function (select) {
        if (select) {
